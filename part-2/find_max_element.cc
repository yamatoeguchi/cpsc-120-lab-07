...
///
/// Be aware that max_element() returns an [vector<int>::iterator]
/// (https://en.cppreference.com/w/cpp/container/vector) and to extract
/// the maximum element, a call to [distance()]
/// (https://en.cppreference.com/w/cpp/iterator/distance) is made with
/// \p the_vector beginning and the resulting iterator from max_element().
///
/// \param the_vector The vector of integers created in the main function.
///
/// \returns The maximum/largest value in the vector
...
int main(int argc, char* argv[]) {
  ...
  // Find the maximum
  int maximum_value = FindMaximum(random_numbers);
  // Print the maximum
  cout << "The maximum value in the vector is " << maximum_value << "\n";
  return 0;
}
