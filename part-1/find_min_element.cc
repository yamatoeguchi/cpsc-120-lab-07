...
void FillVector(vector<int>& the_vector, const int nelements,
  ...
  // TODO: Make sure you can read and understand this code! Be able
  // to do this in future labs, quizzes, and exams.
...
/// FindMinimum walks through each location of \p the_vector, finds the
/// smallest value, and returns it.
///
/// Using the C++ standard library's
/// [min_element()](https://en.cppreference.com/w/cpp/algorithm/min_element)
/// find the smallest element in \p the_vector.
///
/// Be aware that min_element() returns an [vector<int>::iterator]
/// (https://en.cppreference.com/w/cpp/container/vector) and to extract
/// the minimum element, a call to [distance()]
/// (https://en.cppreference.com/w/cpp/iterator/distance) is made with
/// \p the_vector beginning and the resulting iterator from min_element().
///
/// \param the_vector The vector of integers created in the main function.
///
/// \returns The minimum/smallest value in the vector
...
int main(int argc, char* argv[]) {
  ...
  cout << "The minimum value in the vector is " << minimum_value << "\n";
  return 0;
}
